                                      ...
                                      Alter_table_ctx *alter_ctx)
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("halt_AT_after_downgrade",
                {
                  const char act[]=
                    "now SIGNAL alter_table_inplace_after_downgrade "
                    "WAIT_FOR continue_AT";
  ...
  */
  ...
  if (alter_info->requested_algorithm != Alter_info::ALTER_TABLE_ALGORITHM_COPY)
  {
    ...
    */
...
#ifdef WITH_WSREP
    std::cout << "before: check_if_supported_inplace_alter: " << inplace_supported << std::endl;
    // Force PXC to use HA_ALTER_INPLACE_SHARED_LOCK_AFTER_PREPARE for all inplace ALTERs
    //
    // i.e, Upgrade to SHARED LOCK when when thread is a WSREP thd, and exec
    // mode is either TOI or replicated and lock acquisition is a variant of
    // NO_LOCK.
    if (WSREP(thd) &&
    ...
    std::cout << "after: check_if_supported_inplace_alter: " << inplace_supported << std::endl;
#endif
